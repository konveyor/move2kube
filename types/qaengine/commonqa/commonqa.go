...
func GetPortsForService(detectedPorts []int32) []int32 {
	...
	if len(detectedPorts) != 0 {
		...
		selectedPortsStr = qaengine.FetchMultiSelectAnswer(common.ConfigServicesKey+common.Delim+common.ConfigPortsForServiceKeySegment, "Select ports to be exposed :", []string{"Select Other if you want to add more ports"}, detectedPortsStr, allDetectedPortsStr)
	}
	if len(selectedPortsStr) == 0 || common.IsStringPresent(selectedPortsStr, qatypes.OtherAnswer) {
		enteredPorts = qaengine.FetchMultilineAnswer(common.ConfigServicesKey+common.Delim+common.ConfigAdditionalPortsForServiceKeySegment, "Enter the ports to be exposed", []string{"Enter each port in a newline"}, "")
	}
