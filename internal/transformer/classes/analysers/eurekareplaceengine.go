					...
					if isEurekaClient {
						...
						for _, path := range javapaths {
							...
							for _, eachline := range txtlines {
								match, _ := regexp.MatchString(".*EnableEurekaClient.*", eachline)
								if match {
									javaWithEureka = append(javaWithEureka, path)
								}
								match, _ = regexp.MatchString(".*@FeignClient.*", eachline)
								if match {
...
func (t *EurekaReplaceEngine) Transform(newArtifacts []transformertypes.Artifact, oldArtifacts []transformertypes.Artifact) ([]transformertypes.PathMapping, []transformertypes.Artifact, error) {
	...
	for _, a := range newArtifacts {
		...
		for _, path := range pomWithEureka {
			...
			*project.Dependencies = append((*project.Dependencies)[:eurekaIndex], (*project.Dependencies)[eurekaIndex+1:]...)
			
			// write modified pom.xml to file
			pombytes, err := xml.MarshalIndent(project, "  ", "    ")
			// pombytes := []byte(fmt.Sprintf("%v", project))
			if err != nil {
