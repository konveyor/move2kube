...
func AskDynamicQuestion(questionObjI interface{}) (interface{}, error) {
	logrus.Trace("start AskDynamicQuestion")
	defer logrus.Trace("end AskDynamicQuestion")
...
func convertMapTToProblem(questionObj types.MapT) (qatypes.Problem, error) {
	logrus.Trace("start convertMapTToProblem")
	defer logrus.Trace("end convertMapTToProblem")
...
func askQuestion(questionObj types.MapT) (interface{}, error) {
	logrus.Trace("start askQuestion")
	defer logrus.Trace("end askQuestion")

	prob, err := convertMapTToProblem(questionObj)
	if err != nil {
		logrus.Errorf("failed to make a QA problem object using the given information: %+v Error: %q", questionObj, err)
		return nil, err
	}
