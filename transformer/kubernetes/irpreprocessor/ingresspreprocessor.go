...
func (opt *ingressPreprocessor) preprocess(ir irtypes.IR) (irtypes.IR, error) {
	for serviceName, service := range ir.Services {
		...
		for portForwardingIdx, portForwarding := range service.ServiceToPodPortForwardings {
			...
			portForwarding.ServiceType = core.ServiceType(qaengine.FetchSelectAnswer(portKeyPart+"servicetype", desc, hints, common.IngressKind, options))
			...
			if string(portForwarding.ServiceType) == common.IngressKind {
				...
				portForwarding.ServiceRelPath = strings.TrimSpace(qaengine.FetchStringAnswer(portKeyPart+"urlpath", desc, hints, portForwarding.ServiceRelPath))
